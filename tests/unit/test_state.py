...
def test_compare_expected_observed_selects_latest_in_scope_as_of() -> None:
    ...
    older = ObservedProcurement(
        "gpu-canonical", Decimal(1), Decimal(1), Decimal(0), Decimal(0),
        Decimal(0), _scope(), datetime(2026, 1, 1, tzinfo=UTC),
        StateFreshness.CURRENT, (evidence,),
    )
    latest = ObservedProcurement(
        "gpu-canonical", Decimal(4), Decimal(3), Decimal(0), Decimal(0),
        Decimal(0), _scope(), datetime(2026, 1, 2, tzinfo=UTC),
        StateFreshness.PARTIAL, (evidence,),
    )
    other_scope = ObservedProcurement(
        "gpu-canonical", Decimal(4), Decimal(4), Decimal(0), Decimal(0),
        Decimal(0), StateScope("tenant", "other-project", "site", "bom-v1"),
        datetime(2026, 1, 2, tzinfo=UTC), StateFreshness.CURRENT, (evidence,),
    )
    future = ObservedProcurement(
        "gpu-canonical", Decimal(4), Decimal(4), Decimal(0), Decimal(0),
        Decimal(0), _scope(), datetime(2026, 1, 3, tzinfo=UTC),
        StateFreshness.CURRENT, (evidence,),
    )
