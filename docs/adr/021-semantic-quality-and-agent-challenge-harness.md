...
Enforce known contracts with required static, unit, contract, integration, regression, package-smoke, and challenge-manifest checks. Put shipped semantic defects into C001-C008 manifests with deterministic oracles and executable known-bad mutations. Current code must pass each oracle, while the equivalent mutation must be rejected in an isolated checkout. Keep AI review advisory: do not require it to arrive, consume a model API credential, or make merge eligibility wait.
